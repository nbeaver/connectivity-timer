...
sys.stdout = os.fdopen(
    sys.stdout.fileno(), sys.stdout.mode, 0
)  # bufsize of 0 is unbuffered.
    ...
    try:
        ...
        if verbose:
            ...
            if running_failure_length:
                sys.stdout.write(
                    prepend_time(
                        "Connection has failed for this long: {0}+/-{1} seconds.\n".format(
                            running_failure_length, loop_time
                        )
                    )
                )
        ...
        if running_failure_length and successful_last_time == True:
            sys.stdout.write(
                prepend_time(
                    "Connection was up for this long: {0}+/-{1} seconds.\n".format(
                        running_connection_length, loop_time
                    )
                )
            )
        ...
        if verbose:
            ...
            sys.stdout.write(
                prepend_time(
                    "Connection has worked for this long: {0}+/-{1} seconds.\n".format(
                        running_connection_length, loop_time
                    )
                )
            )
        ...
        if running_failure_length and successful_last_time == False:
            sys.stdout.write(
                prepend_time(
                    "Connection failed for this long: {0}+/-{1} seconds.\n".format(
                        running_failure_length, loop_time
                    )
                )
            )
